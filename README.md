...
基于 **GitHub Actions + Playwright + 通义千问 AI 视觉** 的 ChmlFrp 每日自动签到方案。每天北京时间 9:00 自动登录 ChmlFrp 面板并完成签到，积分自动到账，全程无需人工干预。
...
- ✅ 签到结果自动截图，可在 GitHub Actions Artifacts 查看
- ✅ 可选：SMTP 邮件通知签到结果

## 工作原理

```text
GitHub Actions (每天9:00触发)
...
| `SMTP_HOST` | ⬜ 可选 | 邮件通知用，不配则跳过 |
| `SMTP_USER` | ⬜ 可选 | 邮件通知用 |
| `SMTP_PASS` | ⬜ 可选 | 邮件通知用（SMTP 授权码） |
| `SMTP_TO` | ⬜ 可选 | 接收通知的邮箱 |
...
1. Actions 运行页底部 **Artifacts → screenshots** 下载截图，查看 `result.png`
2. 或登录 [ChmlFrp 面板](https://panel.chmlfrp.net/home) → 账户页确认积分增加

### 7. 定时任务

工作流已内置 cron：`0 1 * * *`（UTC）＝ 北京时间每天 **9:00**。Fork 后点过一次 Enable workflow 即生效，无需其他配置。
...
### 坑 4：GitHub Actions 截图文字全是乱码（方块/问号）
